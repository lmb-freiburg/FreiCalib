#ifndef H_REPROJECTIONERRORNOINTRINSICNOEXTRINSIC
#define H_REPROJECTIONERRORNOINTRINSICNOEXTRINSIC
...
struct ReprojectionErrorNoIntrinsicNoExtrinsic {
    // Constructor
  ReprojectionErrorNoIntrinsicNoExtrinsic(double observed_x, double observed_y,
      double* cameraTranslation, double* cameraRotation,
      double* cameraFocal, double* cameraPrincipal, double* cameraDist,
      double* modelPoint)
      : observed_x(observed_x), observed_y(observed_y),
      cameraTranslation(cameraTranslation), cameraRotation(cameraRotation),
  ...
  bool operator()(const T* const objectTranslation, // array of length 3
    ...
    T rot[3];
    rot[0] = (T) cameraRotation[0];
    rot[1] = (T) cameraRotation[1];
    rot[2] = (T) cameraRotation[2];
    ceres::AngleAxisRotatePoint(rot, p, p);
    
    // cameraRotation[0, 1, 2] are the translation.
    p[0] = p[0] + cameraTranslation[0];
    p[1] = p[1] + cameraTranslation[1];
    p[2] = p[2] + cameraTranslation[2];  //> Translate the point into the camera coordinate system
  ...
  static ceres::CostFunction* Create(const double observed_x,
      ...
      return (new ceres::AutoDiffCostFunction<ReprojectionErrorNoIntrinsicNoExtrinsic, 2, 3, 3>( // Template argument is #output, #noparam1, #noparam2, ...
          new ReprojectionErrorNoIntrinsicNoExtrinsic(observed_x, observed_y,
              cameraTranslation, cameraRotation, cameraFocal, cameraPrincipal, cameraDist, modelPoint)));
  }
  double observed_x;
  double observed_y;
  double* cameraTranslation; // array of length 2
  double* cameraRotation; // array of length 2
